...
int main( int argc, char** argv )
{
    ...
    input_array = (unsigned int*) malloc ( DATA_LEN * sizeof(unsigned int));
    for (x=0; x<DATA_LEN; x++)
    {
    ...
    int filter_len;
    for ( filter_len =1; filter_len<=FILTER_LEN; filter_len*=2)
    {
        serialDataFirst ( DATA_LEN, input_array, output_array, filter_len, filter_list );
        memset ( output_array, 0, DATA_LEN );
    ...
    /* Functions to implement later as part of the homework */
    ...
    //  parallelLoopOptimized ( DATA_LEN, input_array, output_array, FILTER_LEN, filter_list );
